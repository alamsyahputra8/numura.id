...
$(document).on('click', '.btnaddMorePhoto', function(e){
    e.preventDefault();
    ...
    .done(function(data){
        ...
        $('#namedataaddmore').html(data.nama);
        $('#ad_id').val(data.id_event);
        // $('#dropmore').action = "<?PHP echo base_url(); ?>core/uploadEventImgMore/"+data.id_event+"";
        $(".dropzonemore").attr("action", "./core/uploadEventImgMore/" + data.id_event +"");
			...
			columnDefs: [
				{
                    ...
                    render: function(data, type, full, meta) {
                        if (aksesUpdate=='ada') {
                            var z = `
                            <a class="btn btn-sm btn-clean btn-icon btn-icon-md btnaddMorePhoto" title="Add More Photos" data-toggle="modal" data-target="#addmorephotos" data-id="`+data+`">
                                <i data-toggle="tooltip" title="Add More Photos" class="la la-plus"></i>
                            </a>
...
var KTFormWidgets = function () {
    ...
    var initAddMore = function () {
        $('#saveaddmore').click(function(e) {
            e.preventDefault();
            ...
            form.ajaxSubmit({
                url: "<?PHP echo base_url(); ?>core/insermorephototevent",
                type: "POST",
                beforeSend: function(){ 
                   KTApp.block('#addmorephotos .modal-content', {
                        overlayColor: '#000000',
                        type: 'v2',
                        state: 'success',
                        message: 'Please wait...'
                ...
                success: function(data) {
                    if(data) {
                        ...
                        setTimeout(function() {
                            ...
                            KTApp.unblock('#addmorephotos .modal-content');
                            
                            $('#addmorephotos').modal('toggle');
                            $('#tabledata').DataTable().ajax.reload();
                            $('#formaddmore')[0].reset();
                            var alert = $('#suksesinsert');
                            alert.removeClass('kt-hidden').show();
